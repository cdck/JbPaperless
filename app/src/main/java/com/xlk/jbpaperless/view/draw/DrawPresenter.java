...
import com.xlk.jbpaperless.util.FileUtil;
...
import static com.xlk.jbpaperless.model.Constant.ANNOTATION_FILE_DIRECTORY_ID;
import static com.xlk.jbpaperless.util.ConvertUtil.bs2bmp;
...
public class DrawPresenter extends BasePresenter<DrawContract.View> implements DrawContract.Presenter {
    ...
    public DrawPresenter(DrawContract.View view,Context context) {
        super(view);
        cxt=context;
    }
    ...
    private void delInform(byte[] datas, int type) throws InvalidProtocolBufferException {
        ...
        if (togetherIDs.contains(opermemberid)) {
            ...
            mView.drawAgain(ArtBoard.LocalPathList);
        }
    }
...
//                }
                ...
                int remainWidth = ArtBoard.artBoardWidth - x;//可容许显示文本的宽度
    ...
    private void agreeJoin(byte[] datas) throws InvalidProtocolBufferException {
        ...
        if (srcwbid == mSrcwbid && srcmemid == mSrcmemid) {//发起人的白板标识和人员ID一致
            ...
            whoTip(opermemberid, appContext.getString(R.string.tip_join_the_sharing));
            isSharing = true;
    ...
    public void stopShare() {
        if (isSharing) {
            ...
            jni.broadcastStopWhiteBoard(InterfaceMacro.Pb_MeetPostilOperType.Pb_MEETPOTIL_FLAG_EXIT.getNumber(),
                    appContext.getString(R.string.exit_white_board), GlobalValue.localMemberId, mSrcmemid, mSrcwbid, alluserid);
            isSharing = false;
            mView.updateShareStatus();
            mSrcwbid = 0;
        }
    ...
    public void savePicture(String fileName, boolean isUpload, Bitmap bitmap) {
        ...
        File uploadPicFile = new File(Constant.file_dir, fileName + ".png");
        ...
        if (isUpload) {
            ...
            tupload.schedule(timerTask, 2000);//  2秒后运行上传
        }
    }
}
