...
import com.xlk.jbpaperless.util.ConvertUtil;
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mDrawStatusListener != null) {
                    mDrawStatusListener.onDrawStatus(true);
                }
                downX = event.getX();
                downY = event.getY();
                break;
...
//                LogUtils.d(TAG, "drag -->" + "拖动后画板的宽高：" + currentArtBoardWidth + " , " + currentArtBoardHeight);
    ...
    private void draw(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mDrawStatusListener != null) {
                    mDrawStatusListener.onDrawStatus(false);
                }
                ...
                if (DrawPresenter.isSharing && currentDrawGraphics != DRAW_TEXT && currentDrawGraphics != DRAW_ERASER) {
    ...
    private void addInk(List<PointF> allpt) {
        long srcwbid = DrawPresenter.mSrcwbid;
        int srcmemid = DrawPresenter.mSrcmemid;
    ...
    private void addDrawShape(int type, List<Float> allpt) {
        ...
        jni.addDrawFigure(operid, opermemberid, srcmemid,/*:发起人的人员ID*/
                DrawPresenter.mSrcwbid, utcstamp, type, paintWidth, paintColor, allpt);
    }

    private void eraserPath(float x, float y) {
        for (int i = 0; i < DrawPresenter.LocalSharingPathList.size(); i++) {
            DrawPath drawPath = DrawPresenter.LocalSharingPathList.get(i);
            ...
            while (sc < 1) {
                ...
                if (Math.abs((int) fa[0] - (int) x) <= 20 && Math.abs((int) fa[1] - (int) y) <= 20) {
                    ...
                    long utcstamp = System.currentTimeMillis();
                    //操作ID 发送之前最后一个的操作ID
                    int operid = DrawPresenter.localOperids.get(i);
                    int opermemberid = GlobalValue.localMemberId;
                    int srcmemid = DrawPresenter.mSrcmemid;
                    ...
                    if (DrawPresenter.localOperids.size() > 0) {
                        //已经撤销后就要删除最后一个
                        DrawPresenter.localOperids.remove(i);
                    }
                    initCanvas();
                    LocalPathList.remove(drawPath);//删除指定drawpath
                    DrawPresenter.LocalSharingPathList.remove(i);
                    //删除最后得一个操作，再从新绘制
                    drawAgain(LocalPathList);
                    //将其他人的绘制信息也重新绘制
                    drawAgain(DrawPresenter.pathList);
                    i--;
                }
            }
        }
...
//        float remainWidth = currentArtBoardWidth - fx;//可容许显示文本的宽度
        ...
        if (canSee > 2) {
            ...
            LocalPathList.add(drawPath);
            if (DrawPresenter.isSharing) {//如果当前在共享中，则发送添加字体
                DrawPresenter.LocalSharingPathList.add(drawPath);
    ...
    private void addText(float x, float y, int finalSize, String drawText) {
        ...
        DrawPresenter.localOperids.add(operid);
        jni.addText(operid, GlobalValue.localMemberId, DrawPresenter.launchPersonId, DrawPresenter.mSrcwbid,
    ...
    public void revoke() {
        ...
        if (DrawPresenter.isSharing && DrawPresenter.localOperids.size() > 0) {
            long srcwbid = DrawPresenter.mSrcwbid;
            long utcstamp = System.currentTimeMillis();
            //操作ID 发送之前最后一个的操作ID
            int operid = DrawPresenter.localOperids.get(DrawPresenter.localOperids.size() - 1);
            int opermemberid = GlobalValue.localMemberId;
            int srcmemid = DrawPresenter.mSrcmemid;
            int figuretype = InterfaceMacro.Pb_MeetPostilFigureType.Pb_WB_FIGURETYPE_ZERO.getNumber();
            jni.whiteBoardDeleteRecord(opermemberid, operid, opermemberid, srcmemid, srcwbid, utcstamp, figuretype);
            DrawPresenter.localOperids.remove(DrawPresenter.localOperids.size() - 1);//删除自己同屏时最后产生的操作ID
        }
    }
    ...
    public void clear() {
        ...
        DrawPresenter.LocalSharingPathList.clear();//清空同屏时自己的操作
        DrawPresenter.localOperids.clear();//清空同屏时自己产生的操作ID
        drawAgain(DrawPresenter.pathList);//重新绘制其它用户的操作
        if (DrawPresenter.isSharing) {
            long utcstamp = System.currentTimeMillis();
            int operid = (int) (utcstamp / 10);
            int opermemberid = GlobalValue.localMemberId;
    ...
    public void setDrawStatusListener(DrawStatusListener listener) {
        mDrawStatusListener = listener;
    }

    public interface DrawStatusListener {
        void onDrawStatus(boolean isDrawing);
    }
}
